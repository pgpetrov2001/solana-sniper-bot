...
import { createPoolKeys, LiquidityStateV4JSON, logger, zip } from './helpers/index.ts';
...
export class Wallet {
	...
	async getMultipleTokenSellExecutionInfo(rawMintAddresses: string[], rawAmountsToSell: string[]): Promise<SwapExecutionInfoJSON[]> {
		...
		logger.debug(`Mints match pools ${mints.length == poolsDatas.length && zip(mints, poolsDatas).every(([ mint, { state: { baseMint } } ]) => mint.toString() == baseMint)}`);
