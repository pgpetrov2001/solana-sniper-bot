...
export class MarketCache {
    ...
    public async save(
        marketId: string,
        market: MinimalMarketLayoutV3JSON | MinimalMarketLayoutV3,
        logging = true,
    ): Promise<number> {
    ...
    public async getMultiple(marketsIds: string[]): Promise<MinimalMarketLayoutV3JSON[]> {
        ...
        if (marketsIdsToFetch.length) {
            ...
            const resps = await Promise.all(
                zip(marketsIdsToFetch, marketsToSave).map(([marketId, market]) => this.save(marketId, market, false)),
            );
    ...
    private async fetch(rawMarketId: string): Promise<MinimalMarketLayoutV3JSON> {
        ...
        if (!this.connection) {
            throw new Error(
                `Failed to find market data for marketId ${marketId}, because no connection to an RPC was provided for the market cache.`,
            );
        }
        ...
        return JSON.parse(
            JSON.stringify(MINIMAL_MARKET_STATE_LAYOUT_V3.decode(marketInfo!.data)),
    ...
    private async fetchMultipleWithGraphQL(rawMarketsIds: string[]): Promise<MinimalMarketLayoutV3JSON[]> {
        ...
        const indexOfMarketId = Object.fromEntries(rawMarketsIds.map((marketId: string, i: number) => [marketId, i]));
        const resp = (await this.solanaIndexer.request(MinimalMarketLayoutV3Query, {
        ...
        const unfetchedMarketIds = setDifference(
            rawMarketsIds,
            result.map(([marketId, _]) => marketId),
        );
    ...
    private async fetchMultipleWithRPC(rawMarketsIds: string[]): Promise<MinimalMarketLayoutV3JSON[]> {
        ...
        return marketsInfo.map((marketInfo) =>
            JSON.parse(JSON.stringify(MINIMAL_MARKET_STATE_LAYOUT_V3.decode(marketInfo!.data))),
        ) as MinimalMarketLayoutV3JSON[];
    }
    ...
    private async fetchMultiple(rawMarketsIds: string[]): Promise<MinimalMarketLayoutV3JSON[]> {
        ...
        if (this.solanaIndexer) {
            ...
            throw new Error(
                `Failed to find market data for ${marketsIds.length} market ids, because no connection to an RPC, or GraphQL client to a solana indexer was provided for the market cache.`,
            );
        }
    }
}
