...
export class PoolCache {
    ...
    private async fetch(mint: string): Promise<SavedPool> {
        if (!this.connection || !this.config) {
            throw new Error(
                `Failed to find associated pool with mint ${mint}, because no connection to an RPC was provided for the pool cache.`,
            );
        }
        ...
        const poolData = JSON.parse(JSON.stringify(LIQUIDITY_STATE_LAYOUT_V4.decode(poolAccount.data)));
        return {
    ...
    private async fetchMultiple(mints: string[]): Promise<SavedPool[]> {
        if (!this.solanaIndexer || !this.config) {
            throw new Error(
                `Cannot fetch multiple liquidity pools, because solana indexer was not specified, and fetching many pools one by one is too cost-prohibitive.`,
            );
        }
        logger.trace(
            {},
            `Querying raydium pools with quote ${this.config.quoteToken.symbol} and base one out of ${mints.length} mints...`,
        );
        const resp = (await this.solanaIndexer.request(LiquidityStateV4GraphqlQuery, {
            where: {
                _and: [{ baseMint: { _in: mints } }, { quoteMint: { _eq: this.config.quoteToken.mint.toBase58() } }],
            },
        })) as LiquidityStateV4GraphqlResponse;
        const indexOfMint = Object.fromEntries(mints.map((mint: string, i: number) => [mint, i]));
