...
export class RenouncedFreezeFilter implements Filter {
	...
	async execute(poolKeys: LiquidityPoolKeysV4): Promise<FilterResult> {
		const errorMessage = [this.checkRenounced ? 'mint' : undefined, this.checkFreezable ? 'freeze' : undefined].filter(
			(e) => e !== undefined,
		);
		try {
			...
			const message = [renounced ? undefined : 'mint', !freezable ? undefined : 'freeze'].filter(
				(e) => e !== undefined,
			);
			...
			logger.error(
				...
				`RenouncedFreeze -> Failed to check if creator can ${errorMessage.join(' and ')} tokens`,
			);
		}

		return {
			ok: false,
			message: `RenouncedFreeze -> Failed to check if creator can ${errorMessage.join(' and ')} tokens`,
		};
	}
}
